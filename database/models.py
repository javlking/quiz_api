...
sql.execute('CREATE TABLE IF NOT EXISTS questions (question_id INTEGER PRIMARY KEY AUTOINCREMENT, main_question TEXT,'
            'variant_1 TEXT, variant_2 TEXT, variant_3 TEXT, variant_4 TEXT, correct_answer TEXT,'
            'q_type TEXT, q_set TEXT);')

sql.execute('CREATE TABLE IF NOT EXISTS user_answer (user_id INTEGER, question_id INTEGER, answer TEXT, '
            'correctness BOOLEAN);')

sql.execute('CREATE TABLE IF NOT EXISTS user_statistic (user_id INTEGER, stat INTEGER, level TEXT, q_set TEXT, data DATETIME);')
...
def get_table_of_leaders(level=None):
    ...
    if not level:
        results = sql.execute('SELECT users.username, user_statistic.stat FROM users INNER JOIN user_statistic ORDER BY user_statistic.stat DESC LIMIT(5);')
...
# Регистрация или получение id пользователя
def register_or_get_user_id(user: Register):
    ...
    if checker:
        ...
        sql.execute('INSERT INTO users (user_phone_number, username, reg_date) VALUES (?,?,?);',
                    (user.phone_number, user.name, datetime.now()))
...
# Функция получения вопросов к тесту
...
# Функция проверки ответа от пользователя
def check_answer(q_id: int, user_id: int, user_answer: str):
    ...
    sql.execute('INSERT INTO user_answer VALUES (?,?,?,?);',
                (user_id, q_id, question_answer[0], 1 if question_answer[0] == user_answer else 0))
...
# Функция завершения теста и получения результата
